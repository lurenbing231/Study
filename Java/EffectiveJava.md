...
Java中存在过期引用的概念，过期引用就是指永远不被解除的引用。例如一下代码（栈实现代码中，pop部分的代码）
```java
public class Stack {
...
<div align="center">
 <img src="../picture/Java/列表优先于数组1.png">
</div>

任何情况下不能将String放到Long容器中，但是利用数组在运行时才能发现问题。

原因：数组的协变，在某些情况下，即使某个对象不是数组的基类型，也可以赋值给数组元素。

协变的情况：1、数组是应用数据类型；2、赋值的对象类型和数组基类型之间有隐式转换或显式转换

### 有限制通配符

extends：生产者，consumer：消费者

1、参数化类型表示一个T生产者，例如：Stack中pushAll方法，参数是给Stack使用的，就用<? extends T>

2、参数化类型表示一个T消费者，例如：Stack中popAll方法，是消费Stack参数的，就用<? supper T>
